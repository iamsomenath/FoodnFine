...
public class BadgeDrawable extends Drawable {
    ...
    public BadgeDrawable(Context context) {
        float mTextSize = context.getResources().getDimension(R.dimen.item_offset);

        mBadgePaint = new Paint();
        mBadgePaint.setColor(Color.CYAN);
    ...
    public void draw(Canvas canvas) {
        ...
        float radius = ((Math.max(width, height) / 2)) / 2.5f;
...
//	        	canvas.drawRoundRect(radius, radius, radius, radius, 10, 10, mBadgePaint);
    ...
    public void setCount(String count) {
        mCount = count;
        // Only draw sliderpage1 badge if there are notifications.
